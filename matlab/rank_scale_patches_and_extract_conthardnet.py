...
for batch_idx in range(n_batches):
    ...
    if batch_idx * bs == end:
        break
    data_a = bw_patches[batch_idx * bs: end, :, :, :]
    data_a = data_a.cuda()
    data_a = Variable(data_a, volatile=True)
    scale_a, desc_a = model(data_a)
    patches_saliency[batch_idx * bs: end] = scale_a.data.cpu().numpy().reshape(-1)
patches_saliency = patches_saliency.reshape(len(idxs), -1)

pst = torch.from_numpy(patches_saliency.astype(np.float32))
pst_nms = nms(Variable(pst.unsqueeze(0))).squeeze()
pst_nms[:,0:3] = 1000

pst_nms = torch.abs(pst_nms)
nms_ps = pst_nms.data.cpu().numpy()
am = nms_ps.argmin(axis = 1)

indexes = np.zeros(len(am))
for i in range(len(am)):
    indexes[i] = i*num_scales + am[i];
